 ...
 * ```
 */
export const useMessageHandler = (room, setChatMessages) => {
    const handleDataReceived = useCallback((payload, participant) => {
        try {
            ...
            switch (messageData.type) {
                ...
                case "bot-llm-text":
                    logger.log("%c🔍 DEBUG BOT-LLM-TEXT%c", "background: #ff9800; color: white; padding: 2px 6px; border-radius: 3px; font-weight: bold;", {
                    ...
                    if (messageData.data?.text) {
                        ...
                        logger.log("%c🤖 CONVAI MESSAGE%c %c" + messageData.data.text, "background: #45b7d1; color: white; padding: 2px 6px; border-radius: 3px; font-weight: bold;", "color: #45b7d1; font-weight: bold;", "color: #f7f1e3;");
                    }
                    break;
                ...
                case "user-transcription":
                    if (messageData.data?.text && messageData.data?.final) {
                        ...
                        setChatMessages((prev) => {
                            const alreadyExists = prev.some((msg) => msg.type === "user-llm-text" &&
                            ...
                            logger.log("%c🎤 USER TRANSCRIPTION%c %c" + messageContent, "background: #ffa726; color: white; padding: 2px 6px; border-radius: 3px; font-weight: bold;", "color: #ffa726; font-weight: bold;", "color: #f7f1e3;");
