...
import { RTVIMessage, TriggerPayload } from "../types";
...
export function useRTVITriggerSender() {
  ...
  const sendTriggerMessage = useCallback(async (payload: TriggerPayload) => {
    if (!localParticipant) {
  ...
  const sendTriggerByName = useCallback(async (triggerName: string) => {
    await sendTriggerMessage({ trigger_name: triggerName });
  }, [sendTriggerMessage]);

  const sendTriggerByMessage = useCallback(async (triggerMessage: string) => {
